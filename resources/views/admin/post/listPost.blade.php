    ...
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold">
                <div class="row">
                    <h5 class="m-0 col-md-8 col-xs-12">Danh sách bài viết</h5>
                    <form action="#"
            ...
            <div class="card-body">
                <div class="analytic">
                    <a href="{{ route('index.post') }}" class="text-primary {{ !$status ? 'active-bottom' : '' }}">
                ...
                <form action="{{ route('action.post') }}" method="post">@csrf
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="btn-add">
                            <a class="btn btn-primary" href="{{ route('add.post') }}" role="button">Thêm mới</a>
                        </div>
                        <div class="btn-add--responsive">
                            <a class="btn btn-primary" href="{{ route('add.post') }}" role="button"><i class="fa-solid fa-plus"></i></a>
                        </div>
                    </div>
                    <table class="table table-striped table-checkall table-responsive-sm">
                        ...
                        <tbody>
                            @if ($listPost->total() > 0)
                                @php $temp = (($listPost->currentpage() - 1) * $listPost->perpage() + 1) - 1 @endphp
                                @foreach ($listPost as $post)
                                    ...
                                    <tr>
                                        ...
                                        <td class="wp_thumbnail"><img
                                                src="{{ asset($post->thumbnail) }} "class="img_thumbnail"alt="">
                                        </td>
                                        <td><a
                                                href="{{ route('edit.post', $post->id) }}">{{ Str::limit($post->title, 50) }}</a>
                                        </td>
                                        ...
                                        @endif
                                        <td><span class="text-badge badge badge-success">{{ $post->PostCat->name }}</span>
                                        </td>
                ...
                {{ $listPost->appends(['keyword' => request()->input('keyword'), 'status' => $status])->links() }}
            </div>
        </div>
    </div>
@endsection
