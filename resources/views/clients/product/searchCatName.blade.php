    ...
    <div class="section" id="list-product-wp">
        @if ($listProduct->count() > 0)
            <div class="section-head clearfix">
                <h3 class="section-title fl-left">Có <span class="text-danger">'{{ $listProduct->total() }}'</span> sản phẩm
                    có từ khoá
                    <span class="text-danger">'{{ $getNameCat->name }}'</span> được tìm thấy
                </h3>
            ...
            <div class="section-detail">
                <ul class="list-item clearfix" id="listProduct">
                    @foreach ($listProduct as $product)
                        <li>
                            <a href="{{ route('product.detail',['slug' => $product->slug, 'id'=>$product->id]) }}" title="" class="thumb">
                                <img class="img_item_product" src="{{ asset($product->thumbnail) }}">
                            </a>
                            <a href="{{ route('product.detail',['slug' => $product->slug, 'id'=>$product->id]) }}" title="" class="product-name">
                                {{ $product->name }}
                            </a>
                            <div class="price">
                            ...
                            <div class="action clearfix btn-detail pb-2">
                                <a href="{{ route('product.detail',['slug' => $product->slug, 'id'=>$product->id]) }}" title=""
